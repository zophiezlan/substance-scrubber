...
let isDown = false;
let painting = false;
let brush = 'round'; //default to round brush at start
let paintColor = '#000';
let mouseX_start;
let mouseY_start;
let lastPos;
...
let brushSize = (blurAmount = 50);
const brushAdjustment = 800;
...
const brushSizeDiv = document.getElementById('brushSizeSlider');
brushSizeDiv.onchange = populateBrushSize;

const blurAmountDiv = document.getElementById('blurAmountSlider');
...
function populateBrushSize() {
  ...
  brushSize = Math.floor(
    (document.getElementById('brushSizeSlider').value * biggerDimension) /
      brushAdjustment
  );
  setCursor();
}
...
const brushFormElements = document.forms['brushForm'].elements['useBrush'];
...
function handleMouseUp(e) {
  ...
  lastPos = null;
  if (painting != 'paint') {
    const tempBlurAmount = blurAmount;
    if (painting == 'undo') {
      blurAmount = 0;
    }
...
button.onclick = function () {
  const imageScrubberInfo = document.getElementById('imageScrubberInfo');
